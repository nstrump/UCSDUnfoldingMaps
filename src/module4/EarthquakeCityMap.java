...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(color(200, 50, 255));
		triangle(50, 115, 60, 95, 70, 115);
		fill(color(255, 255, 255));
		ellipse(60, 135, 20, 20);
		//fill(color(0, 0, 255));
		rect(50, 155, 20, 20);
		fill(color(255, 255, 0));
		ellipse(60, 225, 20, 20);
		fill(color(0, 0, 255));
		ellipse(60, 255, 20, 20);
		fill(color(255, 0, 0));
		ellipse(60, 285, 20, 20);
	...
	private void printQuakes() 
	{
		...
		for (Marker cm: countryMarkers) {
			...
			for (int i =0; i<quakeMarkers.size(); i++) {
				Marker m = quakeMarkers.get(i);
				EarthquakeMarker em = (EarthquakeMarker)m;
				String country = (String)m.getProperty("country");
				if (em.isOnLand() && country.equals(countryName)) {
					landCount++;
				}	
				if (!em.isOnLand()) {
					oceanCount++;
			...
			if (landCount>=1) {
				System.out.println(countryName + " has " + landCount + " earthquakes.");
			}
			
		}
		System.out.println("There are " + oceanTotal + " earthquakes total in the oceans.");
